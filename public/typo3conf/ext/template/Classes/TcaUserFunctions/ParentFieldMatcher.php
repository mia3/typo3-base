...
class ParentFieldMatcher {
    ...
    public function checkField($args){
        ...
        if((!isset($parameters[0]) || !isset($parameters[1]))){
            throw new MissingMandatoryParametersException('Missing parameters! First parameter must be the "field_name" which should be checked! Second parameter must be the value to check!');
        }
        $record = $args['record'];
